...
把磁盘上的目录读成**一棵可逐层展开的树**。同一条目录有两种看法，顶栏「样式」切换
（默认前者，选择记在浏览器本地）：

| 样式 | 长什么样 | 适合 |
|---|---|---|
| **知识库树**（默认） | 三角 + 文件夹 / 文件图标 + 名字，层级靠**缩进**表达 | 像 IDE 或知识库的侧栏，条目多时扫得快 |
| 书目录 | 分卷分章、层级编号（1 / 1.1 / 1.1.1）、点线引导，后面跟着条目数与占用大小 | 「这一卷有多厚」一眼可见 |

两套样式共用同一份 DOM，切换只是往 `body` 上加一个类 —— 不重新请求数据、也不重画
整棵树，纯粹是视觉的事。
...
**模块四 · 本地文件列表** —— 目录树，两种看法可切；默认「知识库树」：
三角 + 文件夹 / 文件图标 + 名字，层级靠缩进

![本地文件列表 · 知识库树](docs/screenshots/pw-v0.6-module-filelist-tree.png)

同一个目录，切到「书目录」就是分章编号 + 点线引导 + 条目数与占用大小：

![本地文件列表 · 书目录](docs/screenshots/pw-v0.4-module-filelist.png)

两种样式**共用同一份 DOM**，切换只是换一个类 —— 不重新读盘、也不重画树。
全程只读
...
| 模块四 · 本地文件列表 | ✅ 可用（知识库树 / 书目录 双样式，自带页面，全程只读） |
