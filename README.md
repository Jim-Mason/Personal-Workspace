...
**当前版本：v0.4** · 浏览器打开即用 · 单进程 · 单端口 · 数据不出本机

---

**整体结构** —— 所有东西都从 8731 进来，四个模块挂在同一套鉴权、审计与路径之下：
...
```
   ...
   ┌───────────────┐ ┌───────────────┐ ┌───────────────┐ ┌───────────────┐
   │ 模块一        │ │ 模块四        │ │ 模块二        │ │ 模块三        │
   │ 本机资产清单  │ │ 本地文件列表  │ │ 运维脚本生成  │ │ 工作地址集合  │
   │ /inventory    │ │ /filelist     │ │ /opsgen       │ │ /portal       │
   │ native 同进程 │ │ native 同进程 │ │ 子进程 :8732  │ │ 子进程 :8733  │
   │ （无端口）    │ │ + 自带页面    │ │ + 反向代理    │ │ + 反代 + 网关 │
   └───────────────┘ └───────────────┘ └───────────────┘ └───────┬───────┘
                                                                 │
                                       ┌─────────────────────────┘
...
其余三个模块都不需要构建：模块一、模块四没有前端，模块二是第三方源码原样托管。
...
| 访问令牌走请求头 | 浏览器中误开的其它页面无法直接调接口（Cookie 会被自动携带，所以不用 Cookie） |
...
│       ├── security.py        统一鉴权（Host + 令牌 + 模块票据 + 免令牌白名单）
...
│   └── scripts/
│       ├── check_modules.py   模块内核自检（按模块分组，共 87 项断言）
