...
—— 那条路正好是独立调试用的。参看 `modules/inventory/context.py` 与
`modules/filelist/context.py`。
...
自带界面的模块（如 `modules/filelist/`）拿入口页当首页即可；
...
| 作用域 | 整个中台 | 仅这一个模块的路径 |
| URL 里会出现吗 | **不会**（模块路径下连 `?token=` 都拒收） | 只在进门那一次用 `?_t=`，随后换 Cookie |
...
这属于「统一导航与鉴权」那一阶段要决策的事，见 [ARCHITECTURE.md](ARCHITECTURE.md) 第七节。
...
第 4~10 节按模块给具体用例（opsgen / portal / filelist 各有一套，其它模块记为跳过）。
